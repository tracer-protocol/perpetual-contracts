...
contract Trader is Ownable {
    ...
    function executeTrade(
        ...
        for (uint256 i = 0; i < n; i++) {
            ...
            require(gasBalances[maker] >= gasCost && gasBalances[taker] >= gasCost, 'TDR: Trader has insufficient gas');
            ...
            (, uint256 makerAmount, , uint256 makerFilled, , , ) = dex.orders(makeOrderId);
            (, uint256 takerAmount, , uint256 takerFilled, , , ) = dex.orders(takeOrderId);
    ...
    function grabOrder(
        ...
        uint256 orderId = dex.orderIdByHash(orderHash);
        if (orderId == 0) {
            //Create the order
            return
                ITracerPerpetualSwaps(market).permissionedMakeOrder(
