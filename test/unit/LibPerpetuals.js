    ...
    describe("canMatch", async() => {
        context("when called with different order prices", async() => {
            it("returns false", async() => {
                ...
                let created = 0
                let orderA = [zeroAddress, zeroAddress, priceA, amount, sideA, expires, created]
                let orderB = [zeroAddress, zeroAddress, priceB, amount, sideB, expires, created]
        ...
        context("when called with the same side", async() => {
            it("returns false", async() => {
                ...
                let expires = 3021382897 // large unix timestamp
                let created = 0
                let orderA = [zeroAddress, zeroAddress, price, amount, sideA, expires, created]
                let orderB = [zeroAddress, zeroAddress, price, amount, sideB, expires, created]
                let result = await libPerpetuals.canMatch(orderA, 0, orderB, 0)
                expect(result).to.equal(false)
            })
        })
        ...
        context("when called with an expired order", async() => {
            it("returns false if order a is expired", async() => {
                ...
                let created = 0
                let orderA = [zeroAddress, zeroAddress, price, amount, sideA, expiresA, created]
                let orderB = [zeroAddress, zeroAddress, price, amount, sideB, expiresB, created]
                let result = await libPerpetuals.canMatch(orderA, 0, orderB, 0)
                expect(result).to.equal(false)
            })
        ...
        context("when called with already filled orders", async() => {
            it("returns false if order a is filled", async() => {
                ...
                let orderA = [zeroAddress, zeroAddress, price, amount, sideA, expires, created]
                let orderB = [zeroAddress, zeroAddress, price, amount, sideB, expires, created]
                let result = await libPerpetuals.canMatch(orderA, filledA, orderB, filledB)
                expect(result).to.equal(false)
            })
