...
describe("Trader Shim unit tests", async () => {
    ...
    beforeEach(async () => {
        ...
        for (var i = 0; i < 6; i++) {
            ...
            await account.deposit(web3.utils.toWei("10000"), tracer.address, {
                from: accounts[i],
            });
        }
    ...
    describe("executeTrade", () => {
        context("When input array lengths differ", () => {
            it("reverts", async () => {
                ...
                let signedMakers = await Promise.all(
                    await signOrders(web3, makers, trader.address)
                );
                let signedTakers = await Promise.all(
                    await signOrders(web3, takers, trader.address)
                );
