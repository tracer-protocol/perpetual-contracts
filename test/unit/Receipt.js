...
describe("Receipt: unit tests", async () => {
    ...
    describe("claimReceipts", () => {
            ...
            it("fails", async () => {
                await expectRevert(
                    receipt.claimReceipts(
                        0,
                        [0, 1, 2],
                        await mockTracer.priceMultiplier(),
                        mockTracer.address,
                        accounts[3],
                        { from: accounts[4] }
                    ),
        ...
        context("When the refund is already claimed", () => {
            it("fails", async () => {
                ...
                let receipt2 = await Receipt.new(
                    accounts[4],
                    maxSlippage,
                    gov.address
                );
                ...
                await receipt2.claimReceipts(
                    0,
                    [0, 1, 2],
                    await mockTracer.priceMultiplier(),
                    mockTracerWithCorrectUnits.address,
                    accounts[1],
                    { from: accounts[4] }
                );
                await expectRevert(
                    receipt2.claimReceipts(
                        0,
                        [0, 1, 2],
                        await mockTracer.priceMultiplier(),
                        mockTracerWithCorrectUnits.address,
    ...
    describe("claimEscrow", () => {
        context(
            "When the sender is not the party entitled to the escrowed funds",
            () => {
                it("fails", async () => {
                    await expectRevert(
                        receipt.claimEscrow(0, accounts[3], {
                            from: accounts[4],
                        }),
        ...
        context("When the escrow has already been claimed", () => {
            it("fails", async () => {
                ...
                await receipt.claimEscrow(0, accounts[2], {
                    from: accounts[4],
                });
                await expectRevert(
