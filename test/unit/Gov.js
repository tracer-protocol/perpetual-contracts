...
describe("Gov: unit tests", async () => {
    ...
    describe("stake", () => {
        it("reverts if gov is not approved to transfer", async () => {
            await expectRevert(
                gov.stake(ether("100"), { from: accounts[1] }),
        ...
        context("when delegating votes", () => {
            ...
            it("adds to the staker's stakedAmount", async () => {
                assert.isTrue(
                    ether("25").eq(await gov.getUserStaked(accounts[0]))
                );
            });

            it("adds to the delegated user's delegatedAmount", async () => {
                assert.isTrue(
                    ether("25").eq(await gov.getStakedAndDelegated(accounts[1]))
                );
            });
            ...
            it("additional staked amounts are added to the staker's stakedAmount", async () => {
                ...
                await gov.stake(ether("25"));
                assert.isTrue(
                    ether("50").eq(await gov.getUserStaked(accounts[0]))
                );
            });
            ...
            it("additional staked amounts are added to the delegated user's delegatedAmount", async () => {
                ...
                await gov.stake(ether("25"));
                assert.isTrue(
                    ether("50").eq(await gov.getStakedAndDelegated(accounts[1]))
                );
            });
        });
    });
        ...
        context("after staking", () => {
            ...
            context("when delegating votes", () => {
                ...
                it("removes from the delegated user's delegatedAmount", async () => {
                    ...
                    assert.isTrue(
                        ether("25").eq(
                            await gov.getStakedAndDelegated(accounts[1])
                        )
                    );
                });
                ...
                it("additional withdrawn amounts are removed from the staker's stakedAmount", async () => {
                    ...
                    await gov.withdraw(ether("25"));
                    assert.isTrue(
                        ether("0").eq(await gov.getUserStaked(accounts[0]))
                    );
                });
    ...
    describe("propose", () => {
        ...
        context("with enough staked", () => {
            ...
            it("reverts if no target is specified", async () => {
                await expectRevert(
                    gov.propose([], [sampleProposalData]),
                    "GOV: targets = 0"
                );
            });
            ...
            it("emits a ProposalCreated event", async () => {
                const { receipt } = await gov.propose(
                    [accounts[0]],
                    [sampleProposalData]
                );
    ...
    describe("vote", () => {
        ...
        it("reverts if the proposal hasn't started", async () => {
            await expectRevert(
                gov.voteFor(proposalNum, ether("50"), { from: accounts[1] }),
                "GOV: Warming up"
            );
        });
        ...
        context("after the proposal is ready", () => {
            ...
            it("reverts if called by the proposer", async () => {
                await expectRevert(
                    gov.voteFor(proposalNum, ether("50")),
                    "GOV: Proposer cant vote"
                );
            });

            it("reverts if proposal was passed", async () => {
                await gov.voteFor(proposalNum, ether("50"), {
                    from: accounts[1],
                });
                await expectRevert(
            ...
            it("reverts if proposal was rejected", async () => {
                await gov.voteAgainst(proposalNum, ether("50"), {
                    from: accounts[1],
                });
                await gov.voteAgainst(proposalNum, ether("50"), {
                    from: accounts[2],
                ...
                await expectRevert(
                    gov.voteAgainst(proposalNum, ether("50"), {
                        from: accounts[3],
                    }),
                    "GOV: Proposal note voteable"
                );
            ...
            it("reverts if voting with more tokens than staked", async () => {
                await expectRevert(
                    gov.voteFor(proposalNum, ether("51"), {
                        from: accounts[1],
                    }),
            ...
            it("allows voting both yes and no", async () => {
                await gov.voteAgainst(proposalNum, ether("25"), {
                    from: accounts[1],
                });
                await gov.voteFor(proposalNum, ether("25"), {
                    from: accounts[1],
                });
            });
            ...
            it("vote locks the caller", async () => {
                ...
                await expectRevert(
                    gov.delegate(accounts[0], { from: accounts[1] }),
                    "GOVD: Vote locked"
                );
            });

            it("uses an expected amount of gas", async () => {
                const { receipt } = await gov.voteFor(
                    proposalNum,
                    ether("50"),
                    { from: accounts[1] }
                );
    ...
    describe("removeDelegate", () => {
        ...
        it("removes from the delegated address delegatedAmount", async () => {
            ...
            const stakedAndDelegatedBefore = await gov.getStakedAndDelegated(
                accounts[1]
            );
            ...
            const stakedAndDelegatedAfter = await gov.getStakedAndDelegated(
                accounts[1]
            );
    ...
    describe("acceptDelegates", () => {
        ...
        it("enables the accepting of delegate tokens", async () => {
            ...
            await gov.acceptDelegates({ from: accounts[1] });
            await gov.delegate(accounts[1]);
            const delegatedAmount = await gov.getStakedAndDelegated(
                accounts[1]
            );
    ...
    describe("disableDelegates", () => {
        it("reverts if tokens are already delegated", async () => {
            ...
            await gov.delegate(accounts[1]);
            await expectRevert(
                gov.disableDelegates({ from: accounts[1] }),
