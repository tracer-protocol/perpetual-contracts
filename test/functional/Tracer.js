...
describe("Tracer", async () => {
    ...
    context("Deposit", async () => {
        it("Updates balances and tvl accordingly", async () => {
            ...
            assert.equal(
                balance[0].toString(),
                web3.utils.toWei("500").toString()
            );
            assert.equal(
                contractTokenBal.toString(),
                web3.utils.toWei("500").toString()
            );
    ...
    context("Withdraw", async () => {
        ...
        it("Prevents withdrawing to below the margin percentage", async () => {
            ...
            await tracer.takeOrder(1, web3.utils.toWei("1000"), {
                from: accounts[1],
            });
    ...
    context("Make Order Onchain", async () => {
        it("Creates a successful order (no leverage)", async () => {
            ...
            const tx = await tracer.makeOrder(
        ...
        it("Creates a successful order (8x leverage)", async () => {
            ...
            assert.equal(
                order[0].toString(),
                web3.utils.toWei("4000").toString()
            );
            assert.equal(order[1].toString(), web3.utils.toWei("0").toString());
            assert.equal(order[2].toString(), oneDollar.toString());
            assert.equal(order[3], true);
    ...
    context("Take Order Onchain", async () => {
        it("Fully matches an order successfully", async () => {
            ...
            await tracer.takeOrder(1, web3.utils.toWei("500"), {
                from: accounts[1],
            });
            ...
            let account2 = await account.getBalance(
                accounts[1],
                tracer.address
            );
            ...
            let orderTakerAmount = await tracer.getOrderTakerAmount(
                1,
                accounts[1]
            );
            assert.equal(
                orderTakerAmount.toString(),
                web3.utils.toWei("500").toString()
            );
        });
        ...
        it("Fully matches an order successfully (leveraged)", async () => {
            ...
            assert.equal(
                account2[0].toString(),
                web3.utils.toWei("1500").toString()
            );
            assert.equal(
                account2[1].toString(),
                web3.utils.toWei("-1000").toString()
        ...
        it("Rejects taking a fully matched order", async () => {
            ...
            await expectRevert(
                //Order over 10x leverage
                tracer.takeOrder(2, web3.utils.toWei("3"), {
                    from: accounts[2],
                }),
                "SDX: Order filled"
            );
        ...
        it("Partially matches an order successfully", async () => {
            ...
            //Short order for 3 TEST/USD against placed order
            await tracer.takeOrder(1, web3.utils.toWei("30"), {
                from: accounts[1],
            });
            ...
            let account3 = await account.getBalance(
                accounts[2],
                tracer.address
            );
            ...
            assert.equal(
                account2[0].toString(),
                web3.utils.toWei("530").toString()
            );
            assert.equal(
                account2[1].toString(),
                web3.utils.toWei("-30").toString()
            );
        ...
        it("Rejects taking an expired order", async () => {
            ...
            await expectRevert(
                tracer.takeOrder(1, web3.utils.toWei("3"), {
                    from: accounts[2],
                }),
                "SDX: Order expired"
            );
    ...
    context("Liquidation", async () => {
        ...
        it("Updates balances after liquidation with 0 escrow amount", async () => {
            ...
            await account.deposit(web3.utils.toWei("750"), tracer.address, {
                from: accounts[2],
            });

            await account.liquidate(
                web3.utils.toWei("500"),
                accounts[1],
                tracer.address,
                { from: accounts[2] }
            );
            let account1 = await account.getBalance(
                accounts[1],
                tracer.address
            );
            let account2 = await account.getBalance(
            ...
            assert.equal(
                account2[0].toString(),
                web3.utils.toWei("1750").toString()
            );
            assert.equal(
                account2[1].toString(),
                web3.utils.toWei("-500").toString()
        ...
        it("Updates balances after liquidation with non-zero escrow amount", async () => {
            ...
            const result = await account.liquidate(
                web3.utils.toWei("500"),
                accounts[1],
                tracer.address,
                { from: accounts[2] }
            ...
            assert.equal(
                account2[0].toString(),
                web3.utils.toWei("1566.4384").toString()
            );
            assert.equal(
                account2[1].toString(),
                web3.utils.toWei("-500").toString()
        ...
        it("Only liquidates accounts under margin", async () => {
            ...
            //Short order for 5 TEST/USD against placed order
            await tracer.takeOrder(1, web3.utils.toWei("5"), {
                from: accounts[1],
            });
        ...
        it("Keeps an accurate record of liquidation receipts", async () => {
            ...
            assert.equal(
                liquidatorReceipt[5].toString(),
                web3.utils.toWei("204.3616").toString()
            ); // escrowedAmount
            assert.equal(
                (liquidatorReceipt[6] - liquidatorReceipt[4]).toString(),
                new BN(900)
            ); // releaseTime: 15 mins in secs
        });
        ...
        it("Trader can claim escrowed funds after the 15 minute safety period", async () => {
            ...
            await expectRevert(
                account.claimEscrow(0, { from: accounts[1] }),
            ...
            await expectRevert(
                account.claimEscrow(0, { from: accounts[2] }),
                "ACTL: Not Entitled"
            );
            ...
            assert.equal(
                (balanceAfter[0] - balanceBefore[0]).toString(),
                web3.utils.toWei("183.5616")
            );
        ...
        it("Liquidator can claim escrowed funds with valid receipts", async () => {
            ...
            await tracer.takeOrder(2, web3.utils.toWei("200"), {
                from: accounts[3],
            });
            ...
            await tracer.takeOrder(3, web3.utils.toWei("100"), {
                from: accounts[2],
            });
            ...
            await tracer.takeOrder(4, web3.utils.toWei("200"), {
                from: accounts[3],
            });
            ...
            let traderBalanceBefore = await account.getBalance(
                accounts[1],
                tracer.address
            );

            await account.claimReceipts(0, [2, 3, 4], tracer.address, {
                from: accounts[2],
            });
            ...
            let traderBalanceAfter = await account.getBalance(
                accounts[1],
                tracer.address
            );
            ...
            assert.equal(
                (traderBalanceAfter[0] - traderBalanceBefore[0]).toString(),
                web3.utils.toWei("178.5616")
            );
        });
        ...
        it("Appropriately caps the slippage", async () => {
            ...
            await gov.voteFor(proposalId, web3.utils.toWei("10"), {
                from: accounts[1],
            });
            await time.increase(twoDays + 1);
            ...
            await time.increase(twoDays + 1);
            await gov.voteFor(nextProposalId, web3.utils.toWei("10"), {
                from: accounts[1],
            });
            await time.increase(twoDays + 1);
        ...
        it("Liquidator can not claim escrowed funds with orders that are too old", async () => {
            ...
            await tracer.takeOrder(currentOrderId, web3.utils.toWei("500"), {
                from: accounts[3],
            });
            ...
            currentOrderId++;
            await expectRevert(
                account.claimReceipts(0, [0], tracer.address, {
                    from: accounts[2],
                }),
    ...
    context("Calculates Margin Correctly", async () => {
        it("Gives correct margin on deposits", async () => {
            await account.deposit(web3.utils.toWei("500"), tracer.address);
            let margin = await account.getUserMargin(
                accounts[0],
                tracer.address
            );
        ...
        it("Factors in the gas cost of liquidation while calculating minimum margin", async () => {
            ...
            //Short order for 5 TEST/USD against placed order
            await tracer.takeOrder(1, web3.utils.toWei("750"), {
                from: accounts[1],
            });

            //Check margin of account
            let minMargin0 = await account.getUserMinMargin(
                accounts[0],
                tracer.address
            );
            let minMargin1 = await account.getUserMinMargin(
                accounts[1],
                tracer.address
            );
        ...
        it("Gives correct margin and notional value after trades", async () => {
            await account.deposit(web3.utils.toWei("1000"), tracer.address);
            await account.deposit(web3.utils.toWei("200"), tracer.address, {
                from: accounts[1],
            });
            await oracle.setPrice(oneDollar);
            //Long order for 5 TEST/USD at a price of $1
            ...
            let margin2 = await account.getUserMargin(
                accounts[1],
                tracer.address
            );
            let notional1 = await account.getUserNotionalValue(
                accounts[0],
                tracer.address
            );
            let notional2 = await account.getUserNotionalValue(
                accounts[1],
                tracer.address
            );
        ...
        it("Gives correct margin after price changes", async () => {
            ...
            let margin1 = await account.getUserMargin(
                accounts[0],
                tracer.address
            );
    ...
    context("Settlement", async () => {
        it("Handles complex funding rate settlements", async () => {
            ...
            await tracer.makeOrder(
                web3.utils.toWei("505"),
                new BN("101000000"),
                true,
                sevenDays
            );
            //Short order for 5 TEST/USD against placed order
            await tracer.takeOrder(1, web3.utils.toWei("505"), {
                from: accounts[1],
            });
            ...
            await tracer.takeOrder(2, web3.utils.toWei("505"), {
                from: accounts[3],
            });
            ...
            let account0 = await account.getBalance(
                accounts[0],
                tracer.address
            );
            let account1 = await account.getBalance(
        ...
        it("Handles complex funding rate settlements with insurance funding rate", async () => {
            ...
            //Short order
            await tracer.takeOrder(1, web3.utils.toWei("2000"), {
                from: accounts[1],
            });
            ...
            await tracer.takeOrder(2, web3.utils.toWei("1000"), {
                from: accounts[3],
            });
            ...
            assert.equal(
                account1[0].toString(),
                web3.utils.toWei("-1000.036523").toString()
            );
            //Account 2 --> SHORT has paid same amount
            //1000 + (2000 * 1) - 0.000036523 * 1000
            assert.equal(
                account2[0].toString(),
                web3.utils.toWei("2999.963477").toString()
            );
            //total = 0.073046

            //These fees go into the insurance pools margin account
            let poolBalance = await account.getBalance(
                insurance.address,
                tracer.address
            );
            assert.equal(
                poolBalance[0].toString(),
                web3.utils.toWei("0.073046")
            );
        });
    });
    ...
    context("internal state", async () => {
        it("Keeps track of the rolling hourly average price and oracle price", async () => {
            ...
            await tracer.takeOrder(2, web3.utils.toWei("2"), {
                from: accounts[1],
            });
            ...
            await tracer.takeOrder(3, web3.utils.toWei("1"), {
                from: accounts[1],
            });
            ...
            //Short order for 1 TEST/USD against placed order
            await tracer.takeOrder(4, web3.utils.toWei("1"), {
                from: accounts[1],
            });
            ...
            let currentHour = (await tracer.currentHour()).toNumber();
            let averagePrice = await pricing.getHourlyAvgTracerPrice(
                currentHour - 1,
                tracer.address
            );
            let oracleAverage = await pricing.getHourlyAvgOraclePrice(
                currentHour - 1,
                tracer.address
            );
            ...
            assert.equal(
                avgPrices[0].toString(),
                new BN("250000000").toString()
            );
            assert.equal(
                avgPrices[1].toString(),
                new BN("150000000").toString()
            );
            ...
            //Short order for 1 TEST/USD against placed order
            await tracer.takeOrder(5, web3.utils.toWei("1"), {
                from: accounts[1],
            });
            let fairPriceUpdated = await pricing.fairPrices(tracer.address);
            let avgPricesUpdated = await pricing.get24HourPrices(
                tracer.address
            );
            ...
            let timeValue = avgDerivativePrice
                .sub(avgOraclePrice)
                .div(new BN(90));
            //current oracle price - time value
            let expectedFairPrice = new BN("200000000").sub(timeValue);
            assert.equal(
                fairPriceUpdated.toString(),
                expectedFairPrice.toString()
            );
        });
        ...
        it("Keeps track of the leveraged notional value", async () => {
            ...
            await account.deposit(web3.utils.toWei("2200"), tracer.address, {
                from: accounts[3],
            });
            await oracle.setPrice(oneDollar);
            ...
            //Leveraged notional value = $1000
            let userBalance = await account.getBalance(
                accounts[0],
                tracer.address
            );
            ...
            await tracer.takeOrder(3, web3.utils.toWei("300"), {
                from: accounts[2],
            });
            ...
            await tracer.takeOrder(4, web3.utils.toWei("1200"), {
                from: accounts[0],
            });
