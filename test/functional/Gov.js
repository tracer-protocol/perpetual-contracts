...
describe("Gov", async () => {
    ...
    context("Staking", async () => {
        ...
        it("Users cant withdraw more then their stake", async () => {
            ...
            await expectRevert.unspecified(
                gov.withdraw(web3.utils.toWei("11"), { from: accounts[2] })
            );
        });
    });
    ...
    context("Proposing", async () => {
        ...
        it("Stakers can propose multiple function executions", async () => {
            ...
            await gov.propose(
                ...
                [sampleProposalData, sampleSelfUpdate]
            );
        });
        ...
        it("Below proposal threshold", async () => {
            ...
            await expectRevert(
                gov.propose([gov.address], [sampleProposalData], {
                    from: accounts[2],
                }),
        ...
        it("Target length should be within bounds", async () => {
            ...
            await expectRevert(
                gov.propose(
                    Array(13).fill(gov.address),
                    Array(13).fill(sampleProposalData)
                ),
                "GOV: Targets > max"
            );
        });
    ...
    context("Voting", async () => {
        it("Voting on an already-executed proposal should revert", async () => {
            ...
            await govToken.approve(gov.address, web3.utils.toWei("5"), {
                from: accounts[2],
            });
