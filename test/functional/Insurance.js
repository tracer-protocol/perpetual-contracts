...
describe("Insurance", async () => {
    ...
    context("Stake", async () => {
        ...
        it("Gets the correct ratio of tokens on stake", async () => {
            ...
            await tracerBaseToken.transfer(
                insurance.address,
                web3.utils.toWei("5")
            );

            //Deposit into insurance pool and sync holdings
            await account.depositTo(
                web3.utils.toWei("5"),
                tracers[0].address,
                insurance.address
            );
            ...
            await tokens[0].approve(insurance.address, web3.utils.toWei("10"), {
                from: accounts[1],
            });
            await insurance.stake(web3.utils.toWei("10"), tracers[0].address, {
                from: accounts[1],
            });

            let poolTokensAfter = await insurance.getPoolUserBalance(
                tracers[0].address,
                accounts[1]
            );
    ...
    context("Withdraw", async () => {
        ...
        it("Gets the correct ratio of tokens on withdraw", async () => {
            ...
            let poolTokensBefore = await insurance.getPoolUserBalance(
                tracers[0].address,
                accounts[0]
            );
            ...
            assert.equal(
                marginAfter.sub(marginBefore).toString(),
                web3.utils.toWei("10")
            );
    ...
    context("Rewards", async () => {
        it("Allows rewards to be deposited to a pool and be claimed", async () => {
            ...
            let poolTokenAddr = await insurance.getPoolToken(
                tracers[0].address
            );
        ...
        it("Staking more and withdrawing claims rewards", async () => {
            ...
            //Withdraw
            let balanceBeforeWithdraw = await tracerGovToken.balanceOf(
                accounts[0]
            );
            await insurance.withdraw(web3.utils.toWei("8"), tracers[0].address);
            let balanceAfterWithdraw = await tracerGovToken.balanceOf(
                accounts[0]
            );
            assert.equal(
    ...
    it("Correctly records the insurance fund variables (target and funding rate)", async () => {
        ...
        await tracer.takeOrder(1, web3.utils.toWei("10000"), {
            from: accounts[1],
        });
    ...
    context("Liquidation", async () => {
        it("Keeps one token in the insurance pool when pool is completely drained", async () => {
            ...
            let insuranceBalance = await insurance.getPoolHoldings(
                tracer.address
            );
