...
describe("Trader", async () => {
    ...
    beforeEach(async () => {
        ...
        for (var i = 0; i < 6; i++) {
            await tracer.setUserPermissions(trader.address, true, {
                from: accounts[i],
            });
        }
    ...
    context("Trading", async () => {
        it("Can batch execute trades (simple)", async () => {
            ...
            let signedTakes = await Promise.all(
                await signOrders(web3, takes, trader.address)
            );
            let signedMakes = await Promise.all(
                await signOrders(web3, makes, trader.address)
            );
        ...
        it("Detects replay attacks in the same batch", async () => {
            ...
            let signedTakesNormal = await Promise.all(
                await signOrders(web3, takes, trader.address)
            );
            let signedTakesReplay = await Promise.all(
                await signOrders(web3, takesReplay, trader.address)
            );
            let signedMakes = await Promise.all(
                await signOrders(web3, makes, trader.address)
            );
        ...
        it("Detects replay attacks in the different batches", async () => {
            ...
            let signedMakes2 = await Promise.all(
                await signOrders(web3, makes2, trader.address)
            );
            await account.deposit(web3.utils.toWei("500"), tracer.address);
            await account.deposit(web3.utils.toWei("500"), tracer.address, {
                from: accounts[1],
