...
async function setupOracles() {
    ...
    gasPriceOracle = await GasOracle.new(
        priceOracle.address,
        gasOracle.address
    );
...
async function setupTestToken(accounts) {
    ...
    //Send out 10000 test tokens to each address
    for (var i = 1; i < 6; i++) {
        await testToken.transfer(accounts[i], web3.utils.toWei("10000"), {
            from: accounts[0],
        });
    }
...
async function setupPerpsFactory(insurance, deployer, gov) {
    let factory = await TracerFactory.new(
        insurance.address,
        deployer.address,
        gov.address
    );
...
async function setupReceipt(account, govAddr) {
    let receipt = await Receipt.new(
        account.address,
        new BN("1000000"),
        govAddr
    ); // Just set unlimited max slippage for flexibility in tests
...
async function setupContractsAndTracer(accounts) {
    ...
    for (var i = 0; i < 6; i++) {
        await testToken.approve(account.address, web3.utils.toWei("10000"), {
            from: accounts[i],
        });
    }
...
async function deployMultiTracers(
    ...
    for (var i = 0; i < 4; i++) {
        ...
        await gov.propose([perpsFactory.address], [proposeTracerData]);
        await time.increase(twoDays + 1);
        await gov.voteFor(proposalNum, web3.utils.toWei("10"), {
            from: accounts[1],
        });
    ...
    for (var i = 0; i < 4; i++) {
        for (var j = 0; j < 6; j++) {
            await tokens[i].transfer(accounts[j], web3.utils.toWei("5000"), {
                from: accounts[0],
            });
            await tokens[i].approve(account.address, web3.utils.toWei("5000"), {
                from: accounts[j],
            });
        }
    }
